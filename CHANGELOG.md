...
* debug: 修復 v8 post-mortem 偵錯功能.
* crypto: publicEncrypt 開始支援受密碼保護的私鑰.
* crypto: 雜湊函數速度提升 30%.
* crypto: 增加 privateEncrypt/publicDecrypt 函式.
* errors
  - 更好的 util.inspect 格式化
  - fs 的錯誤訊息更加地詳細. 這需要一個 `NODE_MODULE_VERSION` bump.
  - http.setHeader 的錯誤訊息更加地詳細.
* dep updates:
  - npm: 更新至 2.4.1
  - http-parser: 退回到 2.3.0
  - libuv: 更新至 1.3.0
  - v8: 更新至 4.1.0.14
* http.request: 從選項中繼承屬性.
* 增加迭代(iterable)介面至 buffers (`for (let byte of buffer.values()) { }`)
* fs: 修復在 `fs.createReadStream` 的 fd 漏失. 詳情請見 497fd72 .
* installer: 在 Windows 上安裝完成後，廣播 WM_SETTINGCHANGE 事件，使其他執行中的行程得知 PATH 改變.
* 增加貢獻者:
...
* REPL 中的 Surrogate pair 可能會使終端畫面凍結 (https://github.com/iojs/io.js/issues/690)
* io.js 無法編譯成靜態庫 (https://github.com/iojs/io.js/issues/686)
